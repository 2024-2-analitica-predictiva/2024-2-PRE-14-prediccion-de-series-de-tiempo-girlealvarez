...
# Funcion grafica serie de tiempo
def plot_time_series(df, yt_col="yt_true"):  # crea el grafico
    """Time series plot."""

    plt.figure(figsize=(12, 4))

    # yt_real
    plt.plot(df[yt_col], ".-k", linewidth=0.5, label=yt_col)  # linea negra con puntos
    ...
    # line division
    plt.plot(
        [len(df[yt_col]) - 24, len(df[yt_col]) - 24],  # grafica linea vertical X menos de 24
...
#Funcion add linear adiciona campo trend
...
# add_month crea un mes en la fecha y crea una nueva columna que se llama mes
def add_month_component(df):
    df = df.assign(month=df.index.str[5:7].astype(int))
    return df


# partir los datos, para dejar solo los ultimos 24 datos
